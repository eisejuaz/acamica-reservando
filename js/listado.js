...
Listado.prototype.buscarRestaurante = function (id) {
    ...
    //     }
    // }
    let restoEncontrado = this.restaurantes.find(restaurant => restaurant.id === id);
...
Listado.prototype.obtenerUbicaciones = function () {
    ...
    let ubicacionesSinRepetir = soloUnicosEnArray(ubicaciones);
...
Listado.prototype.obtenerHorarios = function () {
    ...
    // for (var i = 0; i < this.restaurantes.length; i++) {
    //     arraysHorarios.push(this.restaurantes[i].horarios);
    // }
    let arraysHorarios = this.restaurantes.map(restaurant => restaurant.horarios);

    //En este arreglo vamos a poner todos los horarios, uno por uno
    var todosLosHorarios = [];
    arraysHorarios.forEach(function (array) {
        array.forEach(function (hora) {
            todosLosHorarios.push(hora)
        });
    });

    //En este arreglo vamos a poner todos los horarios pero sin repetidos
    let horariosSinRepetir = soloUnicosEnArray(todosLosHorarios);
